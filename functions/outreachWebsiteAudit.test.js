...
describe('full pipeline — analyzer -> findingSelector -> outreachWriter prompt -> outreachQuality', () => {
  it('the outreach prompt built from lightweight-analyzer findings never mentions Growth Audit as the source of those findings', async () => {
    mockPublicDns();
    ...
    // The prompt must explicitly instruct the model never to claim Growth
    // Audit generated these findings (checked as instructions the model is
    // given, not as a claim that would appear if the model ignored them —
    // verifying actual model output requires a live AI call, out of scope
    // for a deterministic unit test).
    expect(prompt).toContain('NOT writing this message as a report generated by Growth Audit');
    expect(prompt).toContain('Never say "I ran your site through it" or "I ran your site through Growth Audit"');
  });
  ...
  it('a plausible generated message body (evidence-based, no banned phrases) passes the existing quality gate using lightweight-analyzer findings', async () => {
    ...
    const body = `Hi Riverside Plumbing team, I'm Dean, a web developer and designer, and I came across your website. I ran a quick check and noticed ${finding.outreachText}. I built a free website audit tool that checks for things like this. Run your free audit here: https://app.dean-da-dev.co.uk/`;
